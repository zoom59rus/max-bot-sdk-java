...
import ru.max.bot.commands.RawCommandLine;
import ru.max.bot.updates.DefaultUpdateMapper;
...
public class MaxBotBase implements MaxBot {
    private static MethodHandle NO_RESPONSE;

    static {
        try {
            NO_RESPONSE = MethodHandles.lookup().findStatic(MaxBotBase.class, "noResponse",
    ...
    public MaxBotBase(String accessToken, Object... handlers) {
        this(MaxClient.create(accessToken), handlers);
    }

    public MaxBotBase(MaxClient client, Object... handlers) {
        this.client = client;
